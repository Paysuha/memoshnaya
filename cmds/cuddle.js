...
if (!message.mentions.users.first()) {
    return message.channel.send(`Укажи пользователя , к которому хочешь ПРИЖАТЬСЯ`)
}
    ...
    const img = cuddle[r];
    request('https://nekos.life/api/v2/img/cuddle', (err, res, body) =>{
        let arr = JSON.parse(body)
        let embed = new RichEmbed()
        .setAuthor(`${message.author.username} прижался(-лась) ${message.mentions.users.first().username}<3`)
        .setImage(arr.url)
