...
if (!message.mentions.users.first()) {
    return message.channel.send(`Укажи пользователя , которого хочешь УДАРИТЬ`)
}
    ...
    const img = slap[r];
    request('https://nekos.life/api/v2/img/slap', (err, res, body) =>{
        let arr = JSON.parse(body)
        let embed = new RichEmbed()
        .setAuthor(`${message.author.username} ударил(а) ${message.mentions.users.first().username}<3`)
        .setImage(arr.url)
