...
if (!message.mentions.users.first()) {
    return message.channel.send(`Укажи пользователя , которого хочешь ПОЩЕКОТАТЬ`)
}
    ...
    const img = tickle[r];
    request('https://nekos.life/api/v2/img/tickle', (err, res, body) =>{
        let arr = JSON.parse(body)
        let embed = new RichEmbed()
        .setAuthor(`${message.author.username} щекочет ${message.mentions.users.first().username}<3`)
        .setImage(arr.url)
